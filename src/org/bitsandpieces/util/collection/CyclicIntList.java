...
public class CyclicIntList
		extends AbstractCyclicList<int[]>
		implements IntList, Cloneable, RandomAccess, java.io.Serializable {
	
	private static final long serialVersionUID = -2897300622158506763L;

	private static final int[] EMPTY = {};

	private static final Transfer<int[], Collection<?>, long[]> CONTAINS = (int[] a, int aOff, Collection<?> b, int bOff, int len, long[] attachment) -> {
	...
	private static final Transfer<int[], Collection<?>, long[]> NOT_CONTAINS = (int[] a, int aOff, Collection<?> b, int bOff, int len, long[] attachment) -> {
		for (int m = aOff + len; aOff < m; aOff++, bOff++) {
			if (!b.contains(a[aOff])) {
				attachment[bOff >>> 6] |= 1L << bOff;
			}
	...
	private static final Transfer<int[], IntCollection, long[]> CONTAINS_INT = (int[] a, int aOff, IntCollection b, int bOff, int len, long[] attachment) -> {
		for (int m = aOff + len; aOff < m; aOff++, bOff++) {
			if (b.containsInt(a[aOff])) {
				attachment[bOff >>> 6] |= 1L << bOff;
			}
		}
	};

	private static final Transfer<int[], IntCollection, long[]> NOT_CONTAINS_INT = (int[] a, int aOff, IntCollection b, int bOff, int len, long[] attachment) -> {
		for (int m = aOff + len; aOff < m; aOff++, bOff++) {
			if (!b.containsInt(a[aOff])) {
				attachment[bOff >>> 6] |= 1L << bOff;
			}
	...
	public CyclicIntList(int initialCapacity) {
		super(0, 0, 0, new int[initialCapacity]);
	}
	...
	private void writeObject(java.io.ObjectOutputStream str) throws java.io.IOException {
		...
		_readChecked(a, a.length, s, this._idx, this._len, 0, str, 0, s,
				(int[] a1, int aOff, java.io.ObjectOutputStream b, int bOff, int len, Object attachment) -> {
	...
	@Override
	final int _arrayLength(int[] array) {
		return array.length;
	}

	@Override
	protected final int[] _newArray(int len) {
		return new int[len];
	}
	...
	private Object[] _toArray(int s, int off, int len) {
		try {
			...
			_read(a, a.length, s, this._idx, this._len, off, b, 0, len,
					(int[] a1, int aOff, Object[] b1, int bOff, int len1, Object attachment) -> {
	...
	private int[] _toIntArray(int s, int off, int len) {
		try {
			int[] a = this._array;
			int[] b = new int[len];
			_read(a, a.length, s, this._idx, this._len, off, b, 0, len);
	...
	private int[] _toIntArray(int s, int off, int len, int[] b) {
		try {
			Objects.requireNonNull(b);
			int[] a = this._array;
			if (b.length < len) {
	...
	private int _get(int s, int off, int len, int index) {
		try {
	...
	private int _set(int s, int off, int len, int index, int element) {
		try {
			_checkIndex(len, index);
			int[] a = this._array;
	...
	private int _indexOf(int[] a, int s, int i, int n, int off, int len, int o) {
		this._iteration++;	// prevent Object#equals() side-effects
		try {
			return _indexOf(a, a.length, s, i, n, off, len, false,
					(int[] array, int i1, int i2, Integer attachment, boolean attachment2) -> {
						int att = attachment;
	...
	private int _lastIndexOf(int[] a, int s, int i, int n, int off, int len, int o) {
		this._iteration++;	// prevent Object#equals() side-effects
		try {
	...
	private void _add(int s, int index, int e) {
		int n = this._len;
		int[] old = this._array;
		int[] a = _ensureInternal(old, old.length, s, this._idx, n, s + 1);	// may modify this._idx
		a[_insertIndex(a, a.length, s, this._idx, n, index)] = e;
		this._state++;
	}

	private int _removeAt(int s, int off, int len, int index) {
		_checkIndex(len, index);
		int i = this._idx;
		int n = this._len;
		int[] a = this._array;
		int x = off + index;
		int old = a[_physicalIndex(a.length, s, i, n, x)];
		_remove(a, a.length, s, i, n, x, 1);
		this._state++;
		return old;
	}

	private boolean _remove(int s, int off, int len, int o) {
		int i = this._idx;
		int n = this._len;
		int[] a = this._array;
	...
	private void _forEach(int s, int off, int len, IntConsumer action) {
		_forEach(s, this._idx, this._len, off, len, action);
	}

	private void _forEach(int s, int i, int n, int off, int len, IntConsumer action) {
		int[] a = this._array;
		_read(a, a.length, s, i, n, off, action, 0, len,
				(int[] src, int srcOff, IntConsumer dest, int destOff, int len1, Object attachment) -> {
	...
	private boolean _containsAll(int s, int off, int len, final Collection<?> c) {
		...
		try {
			...
			return -1 != _indexOf(a, a.length, s, this._idx, this._len, off, len, true,
					(int[] array, int i1, int i2, Collection<?> attachment, boolean attachment2) -> {
						for (int m = i1 + i2; i1 < m; i1++) {
	...
	private boolean _containsAll(int s, int off, int len, final IntCollection c) {
		// neither are empty
		this._iteration++;	// prevent Collection#contains() interference
		try {
			int[] a = this._array;
			return -1 != _indexOf(a, a.length, s, this._idx, this._len, off, len, true,
					(int[] array, int i1, int i2, IntCollection attachment, boolean attachment2) -> {
						for (int m = i1 + i2; i1 < m; i1++) {
							if (!attachment.containsInt(array[i1])) {
								return i1;
							}
						}
						return -1;
	...
	private void _addCyclicList(int s, int index, CyclicIntList cyc, int s2, int offset, int length) {
		int n = this._len;
		int[] old = this._array;
		int[] a = _ensureInternal(old, old.length, s, this._idx, n, s + length);	// may modify this._idx
		int[] b = cyc._array;
	...
	private int _addAll(int s, int index, final IntCollection c) {
		...
		if (CyclicIntList.class == clazz) {
			// wether this list is the other list makes no difference.
			try {
				_addCyclicList(s, index, (CyclicIntList) c, cs, 0, cs);
				return cs;
			} catch (ArrayIndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
			}
		...
		try {
			...
			int n = this._len;
			int[] old = this._array;
			int[] a = _ensureInternal(old, old.length, s, this._idx, n, s + cs);	// may modify this._idx
			_insert(a, a.length, s, this._idx, n, index, it, 0, cs,
					(PrimitiveIterator.OfInt src, int srcOff, int[] dest, int destOff, int len, Object attachment) -> {
						for (int m = destOff + len; destOff < m; destOff++) {
							dest[destOff] = src.nextInt();
						}
					}, null, _CME());
			this._state += cs;
			if (it.hasNext()) {
	...
	private int _addAll(int s, int index, final Collection<? extends Integer> c) {
		// some optimizations for known implementations
		// note that we cannot simply ask for "instanceof CyclicList", because
		// even it is is a sub class, because its logical ordering 
		// characteristics might be different.
		...
		try {
			...
			int[] a = _ensureInternal(old, old.length, s, this._idx, n, s + cs);	// may modify this._idx
			_insert(a, a.length, s, this._idx, n, index, it, 0, cs,
					(Iterator<? extends Integer> src, int srcOff, int[] dest, int destOff, int len, Object attachment) -> {
						for (int m = destOff + len; destOff < m; destOff++) {
	...
	private void _replaceAll(int s, int off, int len, IntUnaryOperator operator) {
		this._iteration++;	// prevent Consumer#accept() interference
		try {
			int[] a = this._array;
			_read(a, a.length, s, this._idx, this._len, off, operator, 0, len,
					(int[] a1, int aOff, IntUnaryOperator b, int bOff, int len1, Object attachment) -> {
						for (int m = aOff + len1; aOff < m; aOff++) {
							a1[aOff] = (b.applyAsInt(a1[aOff]));
						}
					}, null, _CME());
		} finally {
			this._iteration--;
	...
	private boolean _cyclicListEquals(int s1, CyclicIntList g, int s2, int off1, int off2, int len) {
		int[] a = this._array;	// dest
		int[] b = g._array;		// src
		return _intersect(
				a, a.length, s1, this._idx, this._len, off1,
				b, b.length, s2, g._idx, g._len, off2,
				len,
				(int[] a1, int aOff, int[] b1, int bOff, int len1, Object attachment) -> {
					for (int m = aOff + len1; aOff < m; aOff++, bOff++) {
						if (a1[aOff] != b1[bOff]) {
	...
	private int _hashCode(int s, int off, int len) {
		...
		try {
			...
			return _accumulate(a, a.length, s, this._idx, this._len, off, len, 0,
					(int[] array, int off1, int len1, int currentValue) -> {
						for (int m = off1 + len1; off1 < m; off1++) {
							currentValue = currentValue * 31 + array[off1];
						}
						return currentValue;
					}, _CME());
		} finally {
	...
	public CyclicIntList clone() {
		try {
			return (CyclicIntList) super.clone();
	...
	public boolean removeAll(final IntCollection c) {
		...
		//	return _removeAllSubList(s, c, false, CONTAINS, REMOVE) != 0;
		//}
		return _removeAll(s, 0, s, c, false, CONTAINS_INT) != 0;
	}
	...
	public boolean retainAll(final IntCollection c) {
		...
		//	return _removeAllSubList(s, c, true, NOT_CONTAINS, RETAIN) != 0;
		//}
		return _removeAll(s, 0, s, c, true, NOT_CONTAINS_INT) != 0;
	}

	private int _removeAll(int s, int off, int len, Collection<?> c, boolean retain, Transfer<int[], Collection<?>, long[]> contains) {
	...
	private int _removeAll(int s, int off, int len, IntCollection c, boolean retain, Transfer<int[], IntCollection, long[]> contains) {
		// general solution (works even if c is backed by this list)
		this._iteration++;	// prevent Collection#contains() interference
		try {
			int i = this._idx;
	...
	private int _removeIf(int s, int off, int len, IntPredicate filter) {
		this._iteration++;	// prevent Object#equals() interference
		try {
			return _batchRemove(s, this._idx, this._len, off, len, filter, false,
					(int[] array, int i1, int i2, IntPredicate c, boolean comp) -> {
						for (int m = i1 + i2; i1 < m; i1++) {
	...
	private <T> int _batchRemove(int s, int i, int n, int off, int len, T attachment, boolean comp, Evaluator<int[], T> ev) throws ArrayIndexOutOfBoundsException {
	...
	private void _sort(int s, int off, int len, IntComparator c) {
	...
	private String _toString(int s, int off, int len) {
		...
		try {
			...
			_read(a, a.length, s, this._idx, this._len, off, sb, 0, len,
					(int[] a1, int aOff, StringBuilder b, int bOff, int len1, Object attachment) -> {
						for (int m = aOff + len1; aOff < m; aOff++) {
	...
	public boolean equals(Object obj) {
		...
		try {
			...
			if (CyclicIntList.class == clazz) {
				CyclicIntList cyc = (CyclicIntList) obj;
				return _cyclicListEquals(s, cyc, s, 0, 0, s);
			}
			if (list instanceof IntList) {
				return _equals(s, 0, s, (IntList) list);
			}
	...
	private boolean _equals(int s, int off, int len, List<?> list) {
		...
		int x = _indexOf(a, a.length, s, this._idx, this._len, off, len, true,
				(int[] array, int off1, int len1, Iterator<?> attachment1, boolean attachment2) -> {
					for (int m = off1 + len1; off1 < m; off1++) {
						Object y = attachment1.next();
						if (!(y instanceof Integer) || (int) y != array[off1]) {
							return off1;
						}
					}
					return -1;
	...
	private boolean _equals(int s, int off, int len, IntList list) {
		PrimitiveIterator.OfInt it = list.iterator();
		int[] a = this._array;
		int x = _indexOf(a, a.length, s, this._idx, this._len, off, len, true,
				(int[] array, int off1, int len1, PrimitiveIterator.OfInt attachment1, boolean attachment2) -> {
					for (int m = off1 + len1; off1 < m; off1++) {
						if (array[off1] != attachment1.nextInt()) {
							return off1;
						}
					}
					return -1;
		...
		return new AbstractSplit() {
			...
			private int bind(int i, Object action) {
				if (i < 0) {
					...
					this.index = 0;
					this.maxIndex = CyclicIntList.this._size;
					this.state = CyclicIntList.this._state;
					return 0;
				}
			...
			public final void forEachRemaining(IntConsumer action) {
				...
				CyclicIntList.this._iteration++;
				try {
					_forEach(CyclicIntList.this._size, x, m - x, action);
					this.index = m;
					_checkState(this.state);
				} finally {
			...
			public final Spliterator.OfInt trySplit() {
				...
				if (i < 0) {
					// not yet bound
					int m = CyclicIntList.this._size;
					long st = CyclicIntList.this._state;
					this.index = 0;
					this.state = st;
					int k = m >>> 1;
					if (k == 0) {
					...
					int j = m - k;
					this.maxIndex = j;
					return new BoundSplit(CyclicIntList.this._array, m, CyclicIntList.this._idx, CyclicIntList.this._len, j, k, st);
				}
				...
				return k == 0 ? null : new BoundSplit(CyclicIntList.this._array,
						CyclicIntList.this._size, CyclicIntList.this._idx, CyclicIntList.this._len,
			...
			public final long estimateSize() {
				...
				if (i < 0) {
					this.index = 0;
					this.state = CyclicIntList.this._state;
					return (long) (this.maxIndex = CyclicIntList.this._size);
				}
	...
	private final class BoundListItr implements IntListIterator {
		...
		@Override
		public final void setInt(int e) {
			try {
				int r = this.ret;
				if (r < 0) {
				...
				_checkState(this.state);
				int[] a = CyclicIntList.this._array;
				a[_physicalIndex(a.length, CyclicIntList.this._size, CyclicIntList.this._idx, CyclicIntList.this._len, r)] = e;
			} catch (ArrayIndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
			}
		}
		...
		@Override
		public final void addInt(int e) {
			try {
				_failDuringIteration();
				long exp = this.state;
				_checkState(exp);
				int i = this.index;
				_add(CyclicIntList.this._size, i, e);
				this.state = exp + 1;
				this.index = i + 1;
				this.ret = -1;
				this.maxIndex++;
		...
		public final void remove() {
			try {
				...
				long exp = this.state;
				_checkState(exp);
				_remove(CyclicIntList.this._size, r, 1);
				this.state = exp + 1;
				this.index = r;
				this.ret = -1;
				this.maxIndex--;
	...
	private abstract class AbstractSplit implements Spliterator.OfInt {
	...
	private final class BoundSplit extends AbstractSplit implements Spliterator.OfInt {

		private final int[] _a;
		private final int _s, _i, _n;	// no need to re-read them at all
		private final long state;

		private BoundSplit(int[] a, int s, int i, int n, int index, int len, long state) {
			this._a = a;
			this._s = s;
			this._i = i;
			this._n = n;
	...
	private final class SubList implements IntList, RandomAccess {
		...
		private CyclicIntList owner() {
			return CyclicIntList.this;
		}
		...
		public void clear() {
			try {
				...
				int len = this.length;
				if (len != 0) {
					int s = CyclicIntList.this._size;
		...
		public Object[] toArray() {
			checkState();
			return _toArray(CyclicIntList.this._size, this.offset, this.length);
		}

		@Override
		public <T> T[] toArray(T[] a) {
			checkState();
			return _toArray(CyclicIntList.this._size, this.offset, this.length, a);
		}

		@Override
		public int[] toIntArray() {
			checkState();
			return _toIntArray(CyclicIntList.this._size, this.offset, this.length);
		}

		@Override
		public int[] toIntArray(int[] a) {
			checkState();
			return _toIntArray(CyclicIntList.this._size, this.offset, this.length, a);
		}

		@Override
		public int getInt(int index) {
			checkState();
			return _get(CyclicIntList.this._size, this.offset, this.length, index);
		}

		@Override
		public int setInt(int index, int element) {
			checkState();
			return _set(CyclicIntList.this._size, this.offset, this.length, index, element);
		}

		@Override
		public int indexOfInt(int o) {
			checkState();
			return _indexOf(CyclicIntList.this._array, CyclicIntList.this._size, CyclicIntList.this._idx, CyclicIntList.this._len,
		...
		public int lastIndexOfInt(int o) {
			checkState();
			return _lastIndexOf(CyclicIntList.this._array, CyclicIntList.this._size, CyclicIntList.this._idx, CyclicIntList.this._len,
					this.offset, this.length, o);
		}

		@Override
		public boolean containsInt(int o) {
			return indexOfInt(o) != -1;
		}
		...
		public boolean addInt(int e) {
			try {
				...
				long exp = this.state;
				_checkState(exp);
				_add(CyclicIntList.this._size, this.offset + this.length++, e);
				this.state = exp + 1;
				return true;
			} catch (ArrayIndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
		...
		public void addInt(int index, int element) {
			try {
				...
				_add(CyclicIntList.this._size, this.offset + index, element);
				this.state = exp + 1;
				this.length = len + 1;
			} catch (ArrayIndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
		...
		public int removeIntAt(int index) {
			try {
				...
				_checkState(exp);
				int len = this.length;
				int old = _removeAt(CyclicIntList.this._size, this.offset, len, index);
				this.state = exp + 1;
				this.length = len - 1;
				return old;
			} catch (ArrayIndexOutOfBoundsException ex) {
		...
		public boolean removeInt(int o) {
			try {
				...
				_checkState(exp);
				int len = this.length;
				if (_remove(CyclicIntList.this._size, this.offset, len, o)) {
					this.state = exp + 1;
					this.length = len - 1;
					return true;
				}
		...
		public void forEach(IntConsumer action) {
			...
			CyclicIntList.this._iteration++;
			try {
				_forEach(CyclicIntList.this._size, this.offset, this.length, action);
			} finally {
				CyclicIntList.this._iteration--;
			}
		}
		...
		public boolean containsAll(Collection<?> c) {
			...
			return _containsAll(CyclicIntList.this._size, off, len, c);
		}
		...
		public boolean addAll(Collection<? extends Integer> c) {
			...
			int len = this.length;
			int n = _addAll(CyclicIntList.this._size, this.offset + len, c);
			this.state = exp + n;
			this.length = len + n;
			return true;
		}
		...
		public boolean addAll(int index, Collection<? extends Integer> c) {
			...
			int n = _addAll(CyclicIntList.this._size, this.offset + index, c);
			this.state = exp + n;
			this.length = len + n;
			return true;
		}
		...
		public boolean removeAll(Collection<?> c) {
			...
			int r = _removeAll(CyclicIntList.this._size, this.offset, len, c, false, CONTAINS);
			if (r == 0) {
				return false;
			}
			this.state = exp + r;
		...
		public boolean removeAll(IntCollection c) {
			...
			if (this == c || this.owner() == c) {
				// remove self
				_remove(CyclicIntList.this._size, this.offset, len);
				this.state = exp + len;
				this.length = 0;
				return true;
			}
			...
			// further optimizations for other sublist are only marginally effective
			int r = _removeAll(CyclicIntList.this._size, this.offset, len, c, false, CONTAINS_INT);
			if (r == 0) {
				return false;
			}
			this.state = exp + r;
		...
		public boolean retainAll(Collection<?> c) {
			...
			// further optimizations for other sublist are only marginally effective
			int r = _removeAll(CyclicIntList.this._size, this.offset, len, c, true, NOT_CONTAINS);
			if (r == 0) {
				return false;
			}
			this.state = exp + r;
		...
		public boolean retainAll(IntCollection c) {
			...
			// further optimizations for other sublist are only marginally effective
			int r = _removeAll(CyclicIntList.this._size, this.offset, len, c, true, NOT_CONTAINS_INT);
			if (r == 0) {
				return false;
			}
			this.state = exp + r;
		...
		public boolean removeIf(IntPredicate filter) {
			...
			int n = _removeIf(CyclicIntList.this._size, this.offset, len, filter);
			if (n == 0) {
				return false;
			}
			this.state = exp + n;
		...
		public void replaceAll(IntUnaryOperator operator) {
			Objects.requireNonNull(operator);
			_checkState(this.state);
			_replaceAll(CyclicIntList.this._size, this.offset, this.length, operator);
		}

		@Override
		public void sort(IntComparator c) {
			_checkState(this.state);
			_sort(CyclicIntList.this._size, this.offset, this.length, c);
		}
		...
		public Spliterator.OfInt spliterator() {
			long exp = this.state;
			_checkState(exp);
			return new BoundSplit(CyclicIntList.this._array,
					CyclicIntList.this._size, CyclicIntList.this._idx, CyclicIntList.this._len,
		...
		public String toString() {
			_checkState(this.state);
			return _toString(CyclicIntList.this._size, this.offset, this.length);
		}
		...
		public boolean equals(Object obj) {
			...
			CyclicIntList.this._iteration++;	// prevent Object#equals() interference
			try {
				Class<?> clazz = obj.getClass();
				if (SubList.class == clazz) {
					SubList sub = (SubList) obj;
					...
					return _cyclicListEquals(CyclicIntList.this._size, cyc, cyc._size, this.offset, sub.offset, len);
				}
				if (CyclicIntList.class == clazz) {
					if (this.owner() == obj) {
					...
					CyclicIntList cyc = (CyclicIntList) obj;
					return _cyclicListEquals(CyclicIntList.this._size, cyc, cyc._size, this.offset, 0, len);
				}
				if (list instanceof IntList) {
					return _equals(CyclicIntList.this._size, this.offset, this.length, (IntList) list);
				}
				return _equals(CyclicIntList.this._size, this.offset, this.length, list);
			} finally {
				CyclicIntList.this._iteration--;
			}
		}
		...
		public int hashCode() {
			_checkState(this.state);
			return _hashCode(CyclicIntList.this._size, this.offset, this.length);
		}
	}
}
