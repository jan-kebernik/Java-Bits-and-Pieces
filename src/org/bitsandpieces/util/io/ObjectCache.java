 ...
 * <em>equivalent</em> instances (equals() method is <em>not</em> used). Will
 * return a cached instance when available, otherwise use the installed factory
 * method to create a new instance.
 * <p>
 * Releasing instances back into the cache is not strictly necessary, but is
 * highly encouraged. The same instance must not be released into the cache more
 * than once at the same time, as that will lead to undefined behaviour.
 * <p>
 * This class is useful for avoiding expensive object allocation, e.g. for
 * re-usable arrays of the same size.
 * <p>
 ...
 * the very least, have all the characteristics of a new instance (not currently
 * in use by any thread, no danger of being leaked to other threads, etc...).
 *
...
class ObjectCache<T> {
	...
	private final AtomicReference<Node> stack;
	 ...
	 * release an instance that may already be in the pool!
	 *
